...
struct LoginView: View {
    ...
    var body: some View {
        VStack {
            Text("SoulNode Go").font(.largeTitle).bold().padding()
            ...
            VStack(spacing: 15) {
                ...
                Button(action: { SlskdLauncher.shared.startServer(username: user, password: pass) }) {
            ...
            if status.isRunning {
                Button("Enter App") { isLogged = true }.padding().background(.green).foregroundColor(.white).cornerRadius(10)
            }
        }
    }
}
